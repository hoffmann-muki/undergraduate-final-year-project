    ...
    for i in range(x_dim):
        ...
        if params.cov_type == 'rbf':
            ...
            if params.distance == 'Root Manhattan':
                ...
                K = ((1 - params.eps[i]) * np.sinc(0.5 * params.gamma[i] * Lee_metric(T)) + 0.5 * np.eye(T)) \
                    + params.eps[i] * np.eye(T)
            elif params.distance == 'Canberra':
                K = (1 - params.eps[i]) \
            ...
            K = (1 - params.eps[i]) * (Kp * Kg) \
                + params.eps[i] * np.eye(T) # K + 5I to make the matrix positive definite        

        elif params.cov_type == 'cos':

            K = (1 - params.eps[i]) \
                * (np.cos((2*np.pi/params.p) * params.gamma[i] * np.abs(Tdif)) + np.eye(T)) \
                + params.eps[i] * np.eye(T) # K + I to make the matrix positive definite

        elif params.cov_type == 'lin':

            # normalised linear kernel + I to make the matrix positive definite
            K = (1 - params.eps[i]) \
            * (np.array([[0 if i==j else i*j/(T**2) for j in range(1,T+1)] for i in range(1,T+1)]) \
            ...
            # normalised polynomial kernel + I to make the matrix positive definite
            K = (1 - params.eps[i]) \
