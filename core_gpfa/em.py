...
def em(current_params, seq, minVarFrac):
    ...
    for i in range(emMaxIters):
        ...
        for n in range(N):
            sum_Pauto = sum_Pauto + \
                np.sum(seq[n].Vsm, 2) + np.matmul(seq[n].xsm, seq[n].xsm.T)

        # model.stack_attributes('y')
        Y = np.concatenate([trial.y for trial in seq], 1)
        # model.stack_attributes('xsm')
        Xsm = np.concatenate([np.array(trial.xsm) for trial in seq], 1)
        ...
        if current_params.RforceDiagonal:
            ...
            term = np.sum(np.multiply(
                (sum_yxtrans - np.outer(current_params.d, sum_xall)), current_params.C), 1)
            r = np.square(current_params.d) + \
        ...
        if verbose:
            if getLL:
                print('       lik', round(LLi, 3), '(', np.round(tEnd, 3), 'sec iteration)\n')
        else:
            if getLL:
                print('       lik', round(LLi, 3), '\n')

        # Verify that likelihood is growing monotonically - stop when it isn't decreasing anymore
        if i <= 2:
            LLbase = LLi
        elif LLi < LLold:
            print('Data likelihood has decreased from',
                  round(LLold, 3), 'to', round(LLi, 3), '\n')
